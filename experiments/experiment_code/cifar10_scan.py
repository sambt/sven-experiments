...
def cifar10_scan(cfg):
    """
    Hyperparameter scan over batch size, k fraction, and learning rate for CIFAR-10 classification.
    Uses SmallResNet architecture. Hydra passes this function as a partial; cfg holds top-level params.
    """
    ...
    print(f"Starting CIFAR-10 scan with SVD optimizer")
    ...
    for batch_size, k_item, lr, rtol, svd_mode in hparam_scan:
        ...
        OUTPUT_FILE = f"{exp_cfg['output_file']}_bs{batch_size}_width{cfg.experiment.resnet_width}_k{k}_lr{lr}_rtol{rtol}_svd{svd_mode}"
        if use_rmsprop:
    ...
    # run ResNet trainings with other optimizers for comparison
    print("="*80)
    print("Running ResNet trainings with standard optimizers")
    ...
    for batch_size, lr, optim_name in hparam_scan_standard:
        print(f"\nRunning ResNet with batch_size={batch_size}, lr={lr}, optimizer={optim_name}")

        OUTPUT_FILE = f"{exp_cfg['output_file']}_bs{batch_size}_width{cfg.experiment.resnet_width}_lr{lr}_optim{optim_name}"
        OUTPUT_PATH = f"experiment_results/{exp_cfg['name']}/{OUTPUT_FILE}_df.pkl"
        if os.path.exists(OUTPUT_PATH):
            print(f"Output file {OUTPUT_PATH} already exists; skipping this run.")
            continue
