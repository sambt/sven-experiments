...
def mnist_microbatch_scan(cfg):
    ...
    for batch_size, k_item, lr, rtol, svd_mode, microbatch_size, param_fraction in hparam_scan:
        ...
        print(f"\nRunning SVD optimization with batch_size={batch_size}, k={k}, lr={lr}, rtol={rtol}, svd_mode={svd_mode}, microbatch_size={microbatch_size}, param_fraction={param_fraction}")
        if use_rmsprop:
            print(f"  Using RMSprop with alpha={alpha_rmsProp}")

        OUTPUT_FILE = f"{exp_cfg['output_file']}_bs{batch_size}_width{cfg.experiment.mlp_width}_k{k}_lr{lr}_rtol{rtol}_svd{svd_mode}_mb{microbatch_size}_pf{param_fraction}"
        if use_rmsprop:
        ...
        try:
            train_model = FunctionalModelJac(model, loss_fn, device, microbatch_size=microbatch_size, param_fraction=param_fraction)
            ...
            train_loader = DataLoader(dataset.train_dataset, batch_size=batch_size, shuffle=True, generator=torch.Generator().manual_seed(exp_cfg["loader_seed"]), drop_last=True)
            ...
            print(f"  Encountered an error during training; skipping this run.")
