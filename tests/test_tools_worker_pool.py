...
def test_logs_default_to_holystore_not_to_the_nfs_home():
    """O(10^4) runner logs + hydra dirs for the full plan; /n/home11 is a 95 G NFS home
    at 84% and a failed `> $log` looks like a training failure, not a disk-full one."""
    body = open(POOL).read()
    assert "WORKER_LOG_ROOT:-/n/holystore01/" in body
    assert "/n/home11/sambt/iaifi/sv3/slurm_logs" not in body
...
def test_the_items_file_snapshot_stamp_is_logged_and_a_mismatch_shouts(env):
    ...
    other.write_text("# snapshot /n/holystore01/elsewhere/deadbeef_cafe1234\n" + ITEMS)
    proc = _run(env, items=other, timeout=180)
    assert "written for ANOTHER snapshot (/n/holystore01/elsewhere/deadbeef_cafe1234)" \
        in proc.stdout
