...
|---|---|---|---|---|---|---|
| iaifi_gpu_priority | 380 | **2.24 h** | 6.33 h | 10.1 h | 26.6 h | 50 |
| iaifi_gpu | 170 | **4.47 h** | 8.15 h | 10.8 h | 15.6 h | 2 |
...
- `iaifi_gpu_priority` alone: peak **8 jobs on exactly 2 distinct nodes** (`holygpu8a27302`, `holygpu8a27402`), 2026-09-02 12:20.
- `iaifi_gpu` alone: peak **8 jobs on exactly 2 distinct nodes** (`holygpu8a27202`, `holygpu8a27203`), 2026-09-14 13:28.

So the cap is **2 distinct nodes, not 2 jobs and not 2 GPUs** — 8 single-GPU jobs pack onto 2 four-GPU nodes fine.

**The two caps are independent — verified.** At 2026-09-14 13:28:30 the user had **16 single-GPU jobs running on 4 iaifi nodes simultaneously**: `27202`×4 + `27203`×4 under `iaifi_gpu`, `27303`×4 + `27403`×4 under `iaifi_gpu_priority`. So 2 nodes in each QOS = **16 A100-80GB is genuinely reachable**. Confirmed structurally: `sacctmgr` shows two separate QOS each with `MaxTRESPU node=2`.
...
Barely usable. Only **12 allocations in 3 weeks**, peak 4 concurrent, time-weighted mean **0.12 jobs**. Right now: **131/132 GPUs in use, 0 fully-free nodes, 224 pending**. Fairshare: `sshare -U` → `iaifi_lab/sambt  NormShares=0.00778  EffectvUsage=0.0257  FairShare=0.1015` — usage is ~3.3× share, so priority there is weak and getting weaker as you run. Treat `gpu` as **+0–4 opportunistic GPUs**, never as capacity you plan on.
...
Exclusive is instant *if* an iaifi node is fully free, and impossible otherwise. **Right now 0 of 8 iaifi nodes and 0 of 36 `gpu` nodes are fully free.** It also burns one of your two node slots on a single job.

## 6. Current state (as of ~11:45, 2026-09-18)

Nine RUNNING nanoGPT/GPT-2 scan jobs, `sbatch --wrap`, one config each (`[0/1]` progress bar), 12 h limit, `StdOut=/n/home11/sambt/iaifi/sv3/slurm_logs/gpt2-469444{59..67}.out`:

- `iaifi_gpu`: 46944459/64/65 — all on `holygpu8a27203` (1 node, 3 GPUs)
- `iaifi_gpu_priority`: 46944460/61/63 on `holygpu8a27302`, 46944462 on `holygpu8a27402` (2 nodes, 4 GPUs) → **at cap**
- `gpu`: 46944466 on `holygpu8a22302`, 46944467 on `holygpu8a22303`
...
Contention on the 8 iaifi nodes is heavy and long-lived: `strifinopoulos` **47 pending jobs / 141 pending GPUs** (3-GPU, 16–24 h each) and holds 27102×3, 27103×3, 27303×3, 27402×3; `mgerdes` 10 pending and holds 4 GPUs on **3-day** walltimes; `arghya` 2 GPUs (3 d, 1.75 d elapsed); `tshelley2002` 1 GPU (3 d, 2.3 d elapsed); `kfraser` 10 pending. Every iaifi node is at 4/4 or 3/4.
...
|---|---|---|---|---|---|---|---|
| iaifi_gpu + iaifi_gpu_priority | 4+4 | 32 A100-80 | **2 each (independent)** | 3 d | off, tier 4 | 2 | 58 |
| **iaifi_gpu_requeue** | same 8 | 32 A100-80 | **none (QoS=N/A)** | **7 d** | REQUEUE, tier 3, GraceTime=0 | 2 | — |
...
- **Pessimistic: 6.** Equals the measured 7-day average (5.05) plus a little. This is what you get if strifinopoulos's 141-GPU backlog and the three 3-day holds keep the iaifi nodes at 4/4.
- **Expected: 10–11.** 7–8 from the two iaifi QOS (you hold 7 right now; the 16 ceiling only materialises when 4 iaifi nodes have free GPUs) + 1 opportunistic on `gpu` + **~2.5 A100-equiv from gpu_test**. Matches the measured 4-day average of 7.25 plus gpu_test, which you have essentially never used.
- **Optimistic: 18–20.** 16 iaifi (demonstrated for hours on 09-14) + 2–4 on `gpu`. Requires the other five iaifi users to be quiet; do not budget on it.
...
- **`iaifi_gpu_requeue` is the biggest unexploited lever: no node cap on your own lab's 32 A100s, 7-day walltime.** It could plausibly double the expected number. Cost: `PreemptMode=REQUEUE` with `GraceTime=0`, preempted by tier-4 `iaifi_gpu*` jobs — i.e. by your own labmates whenever they queue. Only worth it if the runner appends results per-config and resumes; you have exactly **one** historical job there (`_validate2_rq.sbatch`, 54 s), so it is untested at scale and should be piloted before the campaign, not during.
